...
open('ExCount_wi_OPGainError_cal.mdl');
...
%     A_binary = 2^floor(log(A)/log(2));
    alfa = alfa_arr(i);
    sim('IADC_wi_GainError_cal', t_sim, options); % 2015 Simulink
...
line_type=["--r*","--y*","--g*","--b*","--m*"];
fig_name=["out_no_cal","out_cal_loop","out_cal_gain_loop","out_cal_gain","out_cal_approx"];

for i_fig = 1:5
    figure ('NAME',fig_name(i_fig)); clf; %% All IDC
    for i = 1:length(alfa_arr)
        hold on;
