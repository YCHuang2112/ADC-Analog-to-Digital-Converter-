...
%% User Parameters
% opgain_dB_USER = 80;
opgain_dB_USER = [80:10:100];
SAR_GAIN_ERROR_ARR_USER = [0.98:0.01:1.02];
...
% xlabel('Frequency [Hz]'); ylabel('PSD [V^2/Hz]'); title('Incremental ADC'); axis([0 fnyq/2 -130 0]);
...
for i_opgain = 1:length(opgain)
    ...
    plot(SAR_GAIN_ERROR_ARR,max(snr_iadc_out_for_specific_gain,[],2)','linewidth',0.5,'DisplayName',sprintf("A_o = %4.1fdB",opgain_dB(i_opgain)) ); 
%     txt = sprintf("A_o=%4.1fdB",opgain_dB);
%     lengend_txt(end+1) = {txt};
end
% legend(lengend_txt);
legend;
title ('SQNR  V.S.  SAR\_GAIN\_ERROR \alpha');
xlabel ('SAR\_GAIN\_ERROR \alpha'); ylabel ('SQNR (dB)'); 
hold off;
